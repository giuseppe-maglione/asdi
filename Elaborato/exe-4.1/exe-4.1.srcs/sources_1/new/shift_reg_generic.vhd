...
begin
    ...
    mux0: entity work.mux_4x1
        port map ( data(0) => data_in, data(1) => data_in, data(2) => ff_out(1),
        data(3) => ff_out(2), sel => sel, y => ff_in(0) );
        
    mux1: entity work.mux_4x1
        port map ( data(0) => ff_out(0), data(1) => data_in, data(2) => ff_out(2),
        data(3) => ff_out(3), sel => sel, y => ff_in(1) );
        
    muxN_2: entity work.mux_4x1
        port map ( data(0) => ff_out(N-3), data(1) => ff_out(N-4), data(2) => ff_out(N-1),
        data(3) => data_in, sel => sel, y => ff_in(N-2) );
        
    muxN_1: entity work.mux_4x1
        port map ( data(0) => ff_out(N-2), data(1) => ff_out(N-3), data(2) => data_in,
        data(3) => data_in, sel => sel, y => ff_in(N-1) );        
                
    -- istanziazione dei mux per scegliere quale dato fornire a ciascun flip flop
    mux_generate: for i in 2 to N-3 generate
        mux: entity work.mux_4x1
            port map ( data(0) => ff_out(i-2), data(1) => ff_out(i-1), data(2) => ff_out(i+1), 
            data(3) => ff_out(i+2), sel => sel, y => ff_in(i) );
        ...
        if rst = '1' then       -- reset sincrono
            ...
            if shift_dir = '0' then     -- shift destro
                if shift_num = '0' then
                    ...
                    reg <= data_in & data_in & reg(N-1 downto 2);     -- shifta di 2 e inserisci a sinistra   
                end if;
                ...
                if shift_num = '0' then
                    ...
                    reg <= reg(N-3 downto 0) & data_in & data_in;     -- shifta di 2 e inserisci a destra   
                end if;
