...
signal counter: std_logic_vector(2 downto 0);       -- valore corrente del conteggio
...
M:  entity work.generic_register    
    generic map ( N )
    port map ( clk => clk, rst => rst_sys, load => load_M, data_in => x1, data_out => M_out );

-- istanziazione del registro AQ
AQ: entity work.generic_shiftreg
    generic map ( 2*N+1 )
    port map ( clk => clk, rst => rst_sys, load => load_AQ, shift => shift, data_in => AQ_in, serial_in => AQ_serial, data_out => AQ_out ); 
...
cnt: entity work.generic_counter
    generic map ( N, 3 )
    port map ( clk => clk, rst => rst_sys, count => count, counter => counter );        
    ...
    if rising_edge(clk) then
        ...
        case curr_state is
            ...
            when finish =>
                ...
                result_temp <= AQ_out(2*N downto 1);
                curr_state <= idle;
