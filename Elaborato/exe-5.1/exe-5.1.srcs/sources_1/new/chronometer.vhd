...
--tmp_base: entity work.divisore_frequenza
...
ss_cnt: entity work.counter_mod60
    port map ( clk => tmp, rst => rst, load => set, load_val => ss_set, enable => ss_enable, count => ss, enable_next => ss_ended );
  
-- istanziazione contatore per i minuti
mm_cnt: entity work.counter_mod60
    port map ( clk => tmp, rst => rst, load => set, load_val => mm_set, enable => mm_enable, count => mm, enable_next => mm_ended );

-- istanziazione contatore per le ore
hh_cnt: entity work.counter_mod24
    port map ( clk => tmp, rst => rst, load => set, load_val => hh_set, enable => hh_enable, count => hh, enable_next => hh_ended );  
