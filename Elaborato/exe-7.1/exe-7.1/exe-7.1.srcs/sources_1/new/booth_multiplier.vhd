...
AQ: entity work.generic_shiftreg
    generic map ( 2*N )
    port map ( clk => clk, rst => rst_components, load => load_sr, shift => shift, data_in => AQ_in, data_out => AQ_out );    
...
begin
    ...
    if rising_edge(clk) then
        ...
        case curr_state is
            ...
            when load_data =>
                rst_components <= '0';   
                AQ_in(2*N-1 downto N) <= (others => '0');
                AQ_in(N-1 downto 0) <= x0;
            ...
            when scan =>
                if AQ_out(0) = qmeno1 then
                    ...
                    -- CONTROLLA QUESTA ASSEGNAZIONE
                    AQ_in(2*N-1 downto N) <= addsub_result;
                    AQ_in(N-1 downto 0) <= AQ_out(N-1 downto 0);
                    load_sr <= '1';
                    curr_state <= rshift;
                end if;
